...
const ElectronicsHero = () => {
  ...
  const [showAllProcessor, setShowAllProcessor] = useState(false);
  ...
  const storageCapacity = ["64 GB", "128 GB", "256 GB","1TB SSD Storage"];
  ...
  return (
    <div className="px-45 py-8 bg-[#f4f4f4]">
      <div className="text-sm text-gray-500 mb-4 space-x-2">
        ...
        <span>&gt;</span>
        <span className="text-black font-medium">Electronics</span>
      </div>
      ...
      <div className="flex">
        ...
        <aside className="w-[290px] bg-white p-4 rounded-lg shadow-sm mr-6">
          ...
          <div className="mb-6">
            ...
            {(showAllCapacity ? storageCapacity : storageCapacity.slice(0, 4)).map((option) => (
              <label className="block text-sm" key={option}>
                <input
                  type="checkbox"
            ...
            {storageCapacity.length > 4 && (
              <button
                onClick={() => setShowAllCapacity(!showAllCapacity)}
                className="text-sm font-semibold text-green-800 hover:underline"
              >
                {showAllCapacity ? "See Less" : "See More"}
              </button>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            {(showAllProcessor ? processorOptions : processorOptions.slice(0, 4)).map((option) => (
              <label className="block text-sm" key={option}>
                <input
                  type="checkbox"
            ...
            {processorOptions.length > 4 && (
              <button
                onClick={() => setShowAllProcessor(!showAllProcessor)}
                className="text-sm font-semibold text-green-800 hover:underline"
              >
                {showAllProcessor ? "See Less" : "See More"}
              </button>
            )}
          </div>
